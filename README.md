...
Спецификация OpenAPI 3.0.3 «Регистрационно-контрольная карта» (версия 1.85) описывает домены:  
•	cards – создание, чтение, изменение, удаление РКК; состояния, согласование, ознакомление, уведомления, связывание документов, объединение файлов.
•	group-operations – групповые операции (перенос, удаление, передача в подготовку).
•	my-documents – «Мои документы» (пагинация и фильтры).
•	send-types – виды отправки/получения.
•	resolution-templates – шаблоны резолюций.
•	in-out-organizations – организации корреспонденции.
Безопасность – JWT (Bearer).
API реализовано согласно спецификации, представленной в файле: openapi: '3.0.3' info: version: '1.85', title: 'Регистрационно-контрольная карта'.
Ключевые пути (неполный перечень):
•	POST /commands/{command}/cards – создать РКК по команде.
•	GET /cards | GET /cards/{cardId} | PUT /cards/{cardId} | DELETE /cards/{cardId}.
•	PATCH /cards/{cardId}/state – смена статуса; PATCH /cards/{cardId}/approvals – согласование; PATCH /cards/{cardId}/reviews – ознакомление.
•	POST /cards/{cardId}/notifications – рассылка уведомлений.
•	GET /cards/{journal}/search – journal-поиск (in/out/smdo_*).
•	GET/POST/PUT/DELETE /cards/send-types*, /cards/resolution-templates*, /cards/organizations*.
•	GET /cards/statuses – справочник статусов.
•	GET /cards/{cardId}/files/merge – объединение файлов РКК в PDF.
Документация отображается через Swagger UI. Файл спецификации (sedm_cards.yml) подключён к Swagger UI и доступен для просмотра всех операций API и их тестирования.
На основе OpenAPI-файла сформирован Postman-файл: sed-api.postman
.json.
Файл может быть импортирован в Postman через интерфейс:
Import → File → sed-api.postman.json.

2 РЕЗУЛЬТАТЫ ОЦЕНКИ КАЧЕСТВА КОДА РАЗРАБОТАННОГО ПС

Для анализа качества программного кода использовались следующие инструменты:
•	SonarQube – статический анализ (bugs, vulnerabilities, code smells, duplications, complexity).
•	JaCoCo – покрытие кода модульными тестами.
•	Checkstyle – анализ стиля кода.
•	PMD – поиск ошибок проектирования.
•	SpotBugs – выявление дефектов байткода.
Набор используемых метрик:
•	Покрытие кода (instruction coverage, branch coverage).
•	Количество ошибок (bugs).
•	Количество уязвимостей (vulnerabilities).
•	Code Smells.
•	Цикломатическая и когнитивная сложность.
•	Процент дублирования кода.
•	Количество нарушений стиля.

Проведённый анализ позволил оценить качество серверной части ПС по ключевым метрикам. Полученные значения показывают степень технического долга, устойчивость проекта к ошибкам, качество архитектурных решений и соблюдение стандартов разработки. При необходимости были выявлены проблемные участки: дублирование логики, повышенная сложность отдельных методов, несоответствие стиля кода или потенциальные риски.
...
Unit-тестирование направлено на проверку корректности работы отдельных небольших модулей: методов, сервисов, утилит и компонентов бизнес-логики. Цель подхода – выявление ошибок на уровне минимальных единиц функциональности до их интеграции в общую систему.
При написании unit-тестов использовалась структура AAA (Arrange – Act – Assert):
...
•	AssertJ – гибкие fluent-assertions.
Примеры unit-теста:

@Test
void addCard_ShouldReturnCard_WhenInputIsValid() {
    // Arrange
    Card input = new Card();
    input.setTitle("Test title");
    input.setDocumentTypeId(1L);
    when(cardRepository.save(any())).thenReturn(input);
    // Act
...
Пример демонстрирует проверку бизнес-логики создания регистрационно-контрольной карточки, включая корректность вызова репозитория и возвращаемого значения.


### Интеграционные тесты

Интеграционное тестирование направлено на проверку взаимодействия компонентов системы: контроллеров, сервисов, слоёв доступа к данным и средств авторизации.
Цель – убедиться, что функциональность сервиса корректно работает в реальной среде выполнения.
Инструменты:
•	Spring Boot Test – поднятие контекста приложения;
•	MockMvc – выполнение HTTP-запросов к REST API;
•	ObjectMapper – обработка JSON;
•	(При необходимости) Testcontainers – поднятие контейнера с тестовой БД.
...
class CardControllerIT {
    ...
    @Test
    void getCard_ShouldReturn200AndCard() throws Exception {
...
Тест показывает выполнение реального запроса к эндпоинту /cards/{id} и проверку статуса, структуры и содержания ответа.
